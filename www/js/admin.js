...
document.getElementById('edit-admin-form').addEventListener('submit', async (e) => {
  ...
  // Fetch live password hash directly from Firebase node to avoid local cache desync
  const snap = await firebaseDB.ref('students/' + session.userId + '/password').once('value');
  const livePwdHash = snap.val();

  if (hashedMaster !== livePwdHash) {
